...
class ClockPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    debugPrint('DateTime: ${listenable.value}');
...
class ClockBgPainter extends CustomPainter {
  ...
  _drawLogoText(Canvas canvas, {double offsetX = 0, double offsetY = 0}) {
    _textPainter.text = TextSpan(
      text: 'Toly',
      style: TextStyle(fontSize: radius * 0.2, color: Colors.blue, fontFamily: 'CHOPS')
    );
    _textPainter.layout();
    _textPainter.paint(
      canvas,
  ...
  //绘制四周的刻度, 遍历 60 次, i%5==0 就说明需要绘制长刻度，每次绘制完后，画布旋转 6° 即可
  void drawScale(Canvas canvas) {
    ...
    for(int i = 0; i < count; i++){
      if(i % 5 == 0) {
        _paint..strokeWidth = longLineWidth
        ..color = Colors.blue;
        canvas.drawLine(
        ...
        _paint..strokeWidth = shortLenWidth
        ..color = Colors.black;
        canvas.drawLine(
          Offset(radius - scaleSpace, 0), 
  ...
  void _paintArc(Canvas canvas){
    ...
    ..addArc(
        Rect.fromCenter( center: Offset(-logic1, 0), width: radius*2, height: radius*2),
        10 / 180 * pi,
        pi / 2 - 20 / 180 * pi);
